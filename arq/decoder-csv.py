...
def decode_ais(ais_messages):
    ...
    for message in ais_messages.strip().split('\n'):
        try:
            ...
            # Skip empty payloads to prevent errors
...
# [Rest of decode functions remain the same]
# ...

def create_vessel_dataframe(results):
    ...
    for mmsi, messages in vessels.items():
        ...
        if position_msgs:
            ...
            if 'timestamp' in position_msgs[0]:
                position_msgs.sort(key=lambda x: x['timestamp'])
            
            latest_pos = position_msgs[-1]
            vessel_info.update({
                'latitude': latest_pos['latitude'],
                'longitude': latest_pos['longitude'],
                'last_updated': latest_pos.get('timestamp')
            })
        ...
        for msg in messages:
            if 'ship_type' in msg:
                ...
                vessel_info['type_code'] = msg['ship_type']
                break
        
        vessel_records.append(vessel_info)
...
def process_ais_file(file_content):
    ...
    results = {
        ...
        'vessel_count': len(set(msg['mmsi'] for msg in decoded_messages if 'mmsi' in msg)),
        'messages': decoded_messages
    }
    
    return results

# Here we add decode_payload and all helper functions from the original code
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        # Default output to input filename with .csv extension
        output_file = input_file.rsplit('.', 1)[0] + '.csv'
        
        # If output file specified as second argument, use that instead
        if len(sys.argv) > 2:
        ...
        # Create DataFrame
        df = create_vessel_dataframe(results)
        ...
        print(f"Saved vessel data to {output_file}")
        
        # Also print vessel summary to stdout
        print("\nVessel Summary:")
        for _, vessel in df.iterrows():
            print(f"MMSI: {vessel['mmsi']}")
            if pd.notna(vessel['name']):
                print(f"  Name: {vessel['name']}")
            if pd.notna(vessel['type']):
                print(f"  Type: {vessel['type']}")
            if pd.notna(vessel['latitude']) and pd.notna(vessel['longitude']):
                print(f"  Position: {vessel['latitude']}, {vessel['longitude']}")
            if pd.notna(vessel['speed']):
                print(f"  Speed: {vessel['speed']} knots")
            if pd.notna(vessel['course']):
                print(f"  Course: {vessel['course']}°")
            print(f"  Messages: {vessel['message_count']}")
            print("")
    else:
        print("Usage: python decoder.py input_file.txt [output_file.csv]")